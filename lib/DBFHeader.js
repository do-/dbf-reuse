...
const DBFHeader = class {
	...
	constructor (options) {
		...
		if (lowerCaseFieldNames) this.lowerCaseFieldNames = true
		if (decoder) this.decoder = decoder
		this._epoch = 1900
	}
	...
	parse () {
		...
		let {src, lowerCaseFieldNames, decoder} = this
		...
		let from = 1; for (let pos = 32 ; pos < src.length - 1; pos += 32) {
			...
			let field = new DBFField ({from, decoder, src: src.slice (pos, pos + 31)})
...
DBFHeader.from = async function (is) {

	let tf = new DBFReader ({no_data: true})
